...
![](images/2023-02-26-15-10-50.png)
...
提供了Assertions.assertThrows() ，用来测试方法的异常情况

### 5、超时断言

提供了Assertions.assertTimeout() 为测试方法设置了超时时间

### 6、快速失败

通过 fail 方法直接使得测试失败

## 7.3 前置条件（assumptions）

类似于断言，不同之处在于不满足的断言会使得测试方法失败，而不满足的前置条件只会使得测试方法的执行终止。

可以看成是测试方法执行的前提，当该前提不满足时，就没有继续执行的必要。
...
## 7.4 嵌套测试
 ...
 可以通过 Java 中的内部类和@Nested 注解实现嵌套测试，从而可以更好的把相关的测试方法组织在一起。
...
控制微服务的技术，可以看出各个微服务占用的资源与对其进行监控、追踪、审计、控制等。
